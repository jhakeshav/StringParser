...
public class Util {
    ...
    public static List<String[]> getOrderedWordArray(String fileName, List<String> header) {
        List<String[]> sentences = new ArrayList<String[]>(16);
        try {
            List<String> wordList = getWordList(getFileContent(fileName));
            int headerCount=0, lineHeaderCount=0;
            for (int i = 0; i < wordList.size(); i++) {
                String line = wordList.get(i).trim();
                if (line.equals("")) {
                    continue;
                ...
                if (lineHeaderCount > headerCount) {
                    int addHeader = lineHeaderCount - headerCount;
                    for (int counter=0;counter<addHeader;counter++) {
                        header.add("Word" + (headerCount+ counter + 1));
                    }
                    headerCount = lineHeaderCount;
                }
